...
export default function CreateExamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function submit() {
    ...
    try {
      ...
      router.push(`/groups/${groupId}?tab=1`);
    } catch (err) { setError(formatApiError(err)); }
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <Box className="flex items-center gap-2">
        <Button startIcon={<ArrowBackRounded />} size="small" onClick={() => router.push(`/groups/${groupId}?tab=1`)}>
          Orqaga
        </Button>
      ...
      <Paper className="p-6 space-y-5">
        ...
        <Box className="flex justify-end gap-2 pt-2">
          <Button variant="outlined" onClick={() => router.push(`/groups/${groupId}?tab=1`)}>Bekor qilish</Button>
